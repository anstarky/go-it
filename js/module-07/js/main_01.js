...
/* Используя массив объектов пользователей users, напиши функции которые с помощью перебирающих методов массивов (никаких for, splice и т. д.) выполняют следующие операции. */
...
const getUniqueSkills = users => users
  .reduce((accum, user) => ([...accum, ...user.skills]), [])
  .filter((skill, index, array) => index === array.indexOf(skill))
  .sort((skillA, skillB) => skillA.localeCompare(skillB, 'en'));


// const getUniqueSkills = users => users.filter((v, i, a) => a.indexOf(v) === i);
